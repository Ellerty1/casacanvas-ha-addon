...
class HomeAssistantClient:
    def __init__(self, supervisor_token: str, client: httpx.AsyncClient) -> None:
    ...
    async def list_states(self) -> list[dict[str, Any]]:
        r = await self._client.get(f"{SUPERVISOR_BASE}/states", headers=self._headers, timeout=30)
        r.raise_for_status()
        return r.json()

    async def list_lovelace_dashboards(self) -> list[dict[str, Any]]:
        r = await self._client.get(
            f"{SUPERVISOR_BASE}/lovelace/dashboards",
    ...
    async def get_dashboard_config(self, url_path: str | None) -> dict[str, Any] | None:
        ...
        r = await self._client.get(
            f"{SUPERVISOR_BASE}/lovelace/config",
            headers=self._headers,
            params=params,
            timeout=30,
        )
